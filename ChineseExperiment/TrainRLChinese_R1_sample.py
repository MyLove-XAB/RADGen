...
learning_rate = 0.001
n_experiment = 500  # 180 # 只跑一次 多次的话 每次需要加载rlagent的参数
...
def sample_ac(ac, test=False):
    ...
    if test:
        ...
        nums = len(ac)
        out = tf.reshape(ac, [-1])
        # counts = tf.ones([len(out)])
        # ac = tf.random.stateless_binomial(shape=[len(out)], seed=[123, 456], counts=counts, probs=out)
...
class Agent(tf.keras.Model):
    ...
    def save_model(self, num):
        path = '/share/home/xuaobo/ai_studio/tasks/weights/weights_Chinese/4_sample_weights_rl_ep{}_{}.ckpt'.format(ep_num, num)
...
def train_PG(n_iter, batch_size_rl, n_exp):
    ...
    for i in range(VAL_SIZE//para_size):
        ...
        ac = sample_ac(ac_prob, test=False)  # sample_ac 中包含了tf.math.sigmoid()
    ...
    RLdf.to_csv("/share/home/xuaobo/ai_studio/tasks/res/Chinese/rl_val4/sample_experiment_ep{}_{}.csv".format(ep_num, n_exp))
    ACdf = pd.DataFrame(ac_res, columns=np.linspace(1, sen_num, num=sen_num))
    ACdf.to_csv("/share/home/xuaobo/ai_studio/tasks/res/Chinese/rl_val4/sample_action_ep{}_{}.csv".format(ep_num, n_exp))
    genHead_df = pd.DataFrame(generate_headline, columns=["generate headline"])
    genHead_df.to_csv("/share/home/xuaobo/ai_studio/tasks/res/Chinese/rl_val4/sample_generate_headline_ep{}_"
                      "{}.csv".format(ep_num, n_exp))
