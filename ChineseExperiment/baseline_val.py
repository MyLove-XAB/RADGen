...
TRAIN_SIZE = len(news) # -1: 除了最后一个，所有的训练
...
DATA_SIZE = len(val_news)
val_document = [str(x) for x in val_news['article'][:DATA_SIZE]]
for i in val_document:  # use test data
    a = re.split(r'。|！|？|……|!|\?', i)
...
def transSum(input_document, env, decoder_maxlen=30):
    """
...
for i in tqdm(range(len(inputs)//para_size + 1)):
    ...
    reference = val_news['title'][i * para_size:min((i + 1) * para_size, len(inputs))].to_list()
    res_trans, _ = transSum(input_document=doc, env=transformer, decoder_maxlen=decoder_maxlen)
    ...
    genHead_df.to_csv(
        "/share/home/xuaobo/ai_studio/tasks/res/Chinese/baseline_generate_headline_ep{}.csv".format(ep_num))
    print("iters: {}, takes seconds: {}".format(i, (time.time() - t1)))
    t1 = time.time()

base = pd.DataFrame(rouge_base, columns=['r1f', 'r2f', 'rlf'])
base.to_csv('/share/home/xuaobo/ai_studio/tasks/res/Chinese/val_baseline1_ep{}.csv'.format(ep_num))
print("Save Results")
